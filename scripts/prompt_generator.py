...
# zero-shot prompting
def zero_shot_prompt_generator(
        dataset: Dataset, 
        example: str, 
        labels: list[str]
    ) -> str:
    """
    Generates a zero-shot prompt for predicting the label of a tabular dataset test sample.

    Args:
        dataset (Dataset): Dataset configuration object.
        example (str): String of feature values for the example.
        labels (list[str]): List of possible label values.

    Returns:
        str: Formatted prompt for zero-shot prediction.
    """
...
def zero_shot_cot_prompt_generator(
        dataset: Dataset, 
        example: str, 
        labels: list[str]
    ) -> str:
    """
    Generates a zero-shot chain-of-thought (CoT) prompt for predicting the label of a tabular dataset test sample.
    ...
    Returns:
        str: Formatted prompt for zero-shot CoT prediction.
    """
...
def objective_judge_prompt_generator(
    ...
    """
    Generates a prompt for an objective judge to evaluate the quality of natural language reasoning generated from XAI attributes.

    Args:
        dataset (Dataset): Dataset configuration object.
        prediction (float): Model's predicted label.
        ground_truth (float): True label.
        ft_raw_vals (dict): Raw feature values for the example.
        shap_vals (dict): SHAP values for the example.
        feature_importances (dict): Global feature importances.
        reason (str): Model's reasoning for its prediction.

    Returns:
        str: Formatted prompt for objective judge evaluation.
    """
...
def reasoning_prompt_generator(
        dataset: Dataset, 
        prediction: str, 
        ground_truth: str, 
        ft_raw_vals: str, 
    ...
    """
    Generates a prompt for detailed reasoning based on feature values, feature importances, and SHAP values.

    Args:
        dataset (Dataset): Dataset configuration object.
        prediction (str): Model's predicted label.
        ground_truth (str): True label.
        ft_raw_vals (str): Feature values for the example.
        shap_vals (str): SHAP values for the example.
        feature_importances (str): Global feature importances.

    Returns:
        str: Formatted prompt for reasoning generation.
    """
...
def cot_prompt_generator(
    ...
    """
    Generates a chain-of-thought (CoT) prompt with XAI guidance using decision-making examples, feature importances, and SHAP values.

    Args:
        dataset (Dataset): Dataset configuration object.
        examples (pd.DataFrame): DataFrame of training examples.
        example_predictions (list): List of model predictions for examples.
        feature_importances (dict): Global feature importances.
        example_shap_values (pd.DataFrame): SHAP values for training examples.
        reasoning (dict): Reasoning for each training example.
        test_example (dict): Feature values for the test example.

    Returns:
        str: Formatted CoT prompt for the test example.
    """
